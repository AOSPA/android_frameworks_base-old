...
public class HybridManager {
    ...
    public static final String ACTION_INJECT_PROPS = "INJECT_HYBRID_DUMMY_PROPS";
    ...
    public HybridManager(Context context, IHybridService service) {
        mContext = context;
        mService = service;
        mDefaultProp = new HybridProp(mContext.getPackageName());
    }
    ...
    public synchronized HybridProp setActivePackage(String packageName) {
        HybridProp tmpProp = null;
        try {
            tmpProp = mService.getHybridProp(packageName);
        ...
        if (tmpProp == null) {
            Log.e(TAG, "HYBRID tmpProp is null!");
            return mDefaultProp;
        }
