...
interface ISecureAppsService {
    ...
    void setAppIntent(in String packageName, in Intent intent);
    ...
    void onAppWindowRemoved(in String packageName, in IBinder token);

    void updateAppVisibility(in String packageName);

    void promptIfNeeded(in String packageName, in IBinder token, in boolean isAppWindowVisible);

    boolean isGame(in String packageName);

    boolean isAppOpen(in String packageName);

    oneway void updateSecureAppsConfig(in String packageName, in Configuration newConfig);
}
