...
public class KernelCpuSpeedReader {
     ...
     * If such a case is detected, the failure count gets incremented.
     */
    private void checkForSysFsBug() {
        try (BufferedReader reader = new BufferedReader(new FileReader(mProcFileOnline))) {
            ...
            if (line = reader.readLine() != null) {
                ...
                if (cpuonline > 0)
                        // Increment failure counter if reading stats fails
                        // while the cpu core is online.
                        mFailureCount++;
            }
        } catch (IOException e) {
