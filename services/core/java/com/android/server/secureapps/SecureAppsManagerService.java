...
public class SecureAppsManagerService extends SystemService {
    ...
    SparseArray<ISecureAppsService> mServices = new SparseArray<>();
    ...
    private void readState() {
        ...
        if (service != null) {
            try {
                boolean enabled = mEnabled.get();
                service.readState(enabled);
            } catch (Exception e) {
    ...
    public void onAppWindowRemoved(String packageName, WindowState w) {
        ...
        if (service != null) {
            try {
                IBinder token = w.mClient.asBinder();
                service.onAppWindowRemoved(packageName, token);
            } catch (Exception e) {
    ...
    public void promptIfNeeded(String packageName, WindowState w) {
        ...
        if (service != null) {
            try {
                IBinder token = w.mClient.asBinder();
                boolean isAppWindowVisible = w.isVisibleLw();
                service.promptIfNeeded(packageName, token, isAppWindowVisible);
            } catch (Exception e) {
    ...
    private boolean createService() {
        try {
            ...
            if (intent == null) {
                Slog.d(TAG, "Secure apps service not found");
                return false;
            }
    ...
    private class SecureAppsServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName className, IBinder binder) {
            ...
            ISecureAppsService service = ISecureAppsService.Stub.asInterface(binder);
            if (service != null) {
                    ...
                    try {
                        service.asBinder().linkToDeath(new IBinder.DeathRecipient() {
                            ...
                            public void binderDied() {
                                ...
                                mServices.remove(mUid);
                                if (mUid == mUserId) {
                                    bind(mUid);
                                }
                            }
                        }, 0);
                        mServices.put(mUid, service);
