...
public final class ActivityRecord extends ConfigurationContainer {
    ...
    public void onStartingWindowDrawn(long timestamp) {
        synchronized (mAtmService.mGlobalLock) {
            ...
            if (getIsAppLocked()) {
                mAtmService.blockAppViewIfLocked(packageName);
            }
        }
    }
    ...
    public void onWindowsDrawn(boolean drawn, long timestamp) {
        synchronized (mAtmService.mGlobalLock) {
            mDrawn = drawn;
            final WindowState winState = mAppWindowToken.findMainWindow(false);
            if (winState != null) {
                if (DEBUG_APPLOCK) Log.v(TAG_APPLOCK, "onWindowsDrawn() app: " + packageName + " Token: " + winState.mClient.asBinder());
                mAtmService.showAppLockIfNeeded(packageName, winState.mClient.asBinder());
            }
            if (!drawn) {
                return;
            }
    ...
    public void onWindowsGone() {
        synchronized (mAtmService.mGlobalLock) {
            ...
            if (getIsAppLocked()) {
                final WindowState winstate = mAppWindowToken.findMainWindow(false);
                if (DEBUG_APPLOCK) Log.v(TAG_APPLOCK, "onWindowsGone() app: " + packageName);
                mAtmService.setAppWindowsGone(packageName, (winstate == null) ? null : winstate.mClient.asBinder());
            }
        }
    }
