...
public class ActivityTaskManagerService extends IActivityTaskManager.Stub {
    ...
    public final int startActivityFromRecents(int taskId, Bundle bOptions) {
        ...
        if (r != null) {
            runSecurityCheck();
            if (isSystemSecure() && isAppLocked(r.packageName) && !isAppOpened(r.packageName)) {
    ...
    boolean isSystemSecure() {
        if (mAppLockService == null) return false;
        return mAppLockService.isSystemSecure();
    }

    void runSecurityCheck() {
        if (mAppLockService == null) return;
        mUiHandler.postAtFrontOfQueue(() -> mAppLockService.isSecure());
    }
