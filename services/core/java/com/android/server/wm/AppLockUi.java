...
public class AppLockUi {
    ...
    private final ArrayList<WindowState> mWindowStates = new ArrayList<>();
    ...
    public AppLockUi(Context context, String packageName, int userId, ActivityTaskManagerService s) {
        mContext = context;
        ...
        mPasswordEntry.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch (View v, MotionEvent event) {
                if (mPasswordEntry.hasFocus() && mHasFOD && mImm.isActive(mPasswordEntry)) {
                    ...
                    sAppLockService.stopListeningForFingerprint();
                }
                return false;
            }
        });
    ...
    public boolean isAppWindowsVisible() {
        ...
        for (int i = 0; i < mWindowStates.size(); i++) {
            final WindowState win = mWindowStates.get(i);
            visible |= win.isVisibleLw();
        }
        Log.v(TAG, "isAppWindowsVisible(): packageName=" + mPackageName + " visible: " + visible);
        return visible;
    }

    public void updateAppWindow(WindowState w) {
        Log.v(TAG, "updateAppWindow(): packageName=" + mPackageName + " add window: " + w + " to: " + mWindowStates);
        if (!mWindowStates.contains(w)) {
            mWindowStates.add(w);
        }
    }
    ...
    public void setAppWindowGone(WindowState w) {
        Log.v(TAG, "setAppWindowsGone(): packageName=" + mPackageName + " remove window: " + w + " from: " + mWindowStates);
        if (mWindowStates.contains(w)) {
            mWindowStates.remove(w);
        }
        if (mWindowStates.size() == 0) {
            hide();
            removeBlockView();
        }
    }
    ...
    public boolean updateConfig(Configuration newConfig) {
        ...
        if (mRotation != rot) {
            ...
            inflateViews();
            if (mLayoutChanged) {
                final int N = mWindowStates.size();
                if (N > 0) {
                    show(mWindowStates.get(N-1));
                    return true;
                }
            }
        }
    ...
    public void onAppLaunching() {
        ...
        final int N = mWindowStates.size();
        if (N > 0) show(mWindowStates.get(N-1));
    }

    // Once the app is fully drawn, we switch to the AppLock view
    public void show(WindowState w) {
        reloadPasswordSetup();
        Log.v(TAG, "show(): packageName=" + mPackageName + " add window: " + w + " to: " + mWindowStates);
        if (!mWindowStates.contains(w)) {
            mWindowStates.add(w);
            hide();
        }
        mLayoutParams.token = w.mClient.asBinder();
        addView();
        if (sTimerRunning) {
            setInputEnabled(false);
        } else {
