...
public class AppLockUi {
    ...
    public void setAppLaunching() {
        /*if (!mBlockingAppView) {
            if (DEBUG) Log.v(TAG, "setAppLaunching(): packageName=" + mPackageName + " mShowing:"+mShowing);
            addBlockView();
            mBlockingAppView = true;
            mHandler.postDelayed(() -> {
                if (DEBUG) Log.v(TAG, "removeBlockView() after timeout packageName=" + mPackageName);
                removeBlockView();
            }, mShowing ? 300 : 4000);
            mHandler.post(() -> {
                final int N = mWindowStates.size();
        ...
        if (!mShowing) {
            if (DEBUG) Log.v(TAG, "setAppLaunching(): packageName=" + mPackageName + " mShowing=false");
            final int N = mWindowStates.size();
            if (N > 0) show(mWindowStates.get(N-1));
        }
    }
    ...
    public void show(WindowState w) {
        mHandler.post(() -> reloadPasswordSetup());
