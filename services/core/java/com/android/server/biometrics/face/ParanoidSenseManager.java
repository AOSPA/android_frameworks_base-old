...
public class ParanoidSenseManager {
    ...
    public static final int SENSE_ID = 1108;
    ...
    ISenseServiceReceiver mReceiver = new ISenseServiceReceiver.Stub() {
        ...
        public void onEnrollResult(int faceId, int userId, int remaining) {
            mHandler.post(new Runnable() {
                ...
                public final void run() {
                    mFaceService.super.handleEnrollResult(new Face(
                            mFaceService.getBiometricUtils().getUniqueName(
                            getContext(), userId), faceId, SENSE_ID), remaining);
                }
            });
        }
        ...
        public void onAuthenticated(int faceId, int userId, byte[] token) {
            mHandler.post(new Runnable() {
                ...
                public final void run() {
                    ...
                    mFaceService.super.handleAuthenticated(face, token_AL);
                }
            });
        }
        ...
        public void onAcquired(int userId, int acquiredInfo, int vendorCode) {
            mHandler.post(new Runnable() {
                @Override
                public final void run() {
                    mFaceService.super.handleAcquired(SENSE_ID, acquiredInfo, vendorCode);
                }
            });
        }
        ...
        public void onError(int error, int vendorCode) {
            mHandler.post(new Runnable() {
                @Override
                public final void run() {
                    mFaceService.super.handleError(SENSE_ID, error, vendorCode);
                }
            });
        }
                ...
                public final void run() {
                    if (faceIds.length > 0) {
                        for (int i = 0; i < faceIds.length; i++) {
                            mFaceService.super.handleRemoved(new Face("", faceIds[i], SENSE_ID), (faceIds.length - i) - 1);
                        }
                        return;
                    }
                    mFaceService.super.handleRemoved(new Face("", 0, SENSE_ID), 0);
                }
            });
        }
        ...
        public void onEnumerate(int[] faceIds, int userId) throws RemoteException {
            mHandler.post(new Runnable() {
                ...
                public final void run() {
                    if (faceIds.length > 0) {
                        for (int i = 0; i < faceIds.length; i++) {
                            mFaceService.super.handleEnumerate(new Face("", faceIds[i], SENSE_ID), (faceIds.length - i) - 1);
                        }
                        return;
                    }
                    mFaceService.super.handleEnumerate(null, 0);
                }
            });
        }
        ...
        public void onLockoutChanged(long duration) throws RemoteException {
            if (duration == 0) {
                mFaceService.mCurrentUserLockoutMode = 0;
            } else if (duration == Long.MAX_VALUE) {
                mFaceService.mCurrentUserLockoutMode = 2;
            } else {
                mFaceService.mCurrentUserLockoutMode = 1;
            }
            mHandler.post(new Runnable() {
                @Override
                public final void run() {
    ...
    public int authenticate(int operationId) {
            ...
            mSenseServiceHandler.post(new Runnable() {
                ...
                public final void run() {
                    try {
                        ...
                        FaceService.super.handleError(SENSE_ID, 8, 0);
                    }
                }
            });
            return 0;
    ...
    private boolean createService(int userId) {
        try {
            ...
            boolean result = getContext().bindServiceAsUser(intent, new SenseServiceConnection(userId), 65, UserHandle.of(userId));
            if (result) {
                mBound = true;
            }
            return result;
    ...
    public ISenseService getService(int userId) {
        if (userId == UserHandle.USER_NULL) {
            mService.updateActiveGroup(ActivityManager.getCurrentUser(), null);
        }
        return mServices.get(mCurrentUserId);
    }

    private Intent getServiceIntent() {
        Intent intent = new Intent("com.paranoid.facesense.BIND_SERVICE");
                ...
                synchronized (mServices) {
                    try {
                        ...
                        mHandler.post(new Runnable() {
                            ...
                            public final void run() {
                                if (mServices.size() == 1) {
                                    mService.loadAuthenticatorIds();
                                }
                                mService.updateActiveGroup(mUserId, null);
                                mService.doTemplateCleanupForUser(mUserId);
                            }
                        });
