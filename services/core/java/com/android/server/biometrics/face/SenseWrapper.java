...
import android.hardware.face.sense.ISenseService;
import android.os.IBinder;
import android.os.RemoteException;
import android.util.Log;

import com.android.server.biometrics.face.ParanoidFaceServiceBase.FaceServiceListener;

public class SenseWrapper {

    private static SenseWrapper sInstance;
    private ServiceConnection mConnection = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            synchronized (mLock) {
                Log.d("FaceServiceWrapper", "mConnection onServiceConnected");
                mService = ISenseService.Stub.asInterface(iBinder);
                if (mListener != null) {
    ...
    public ISenseService mService;
    public final Object mLock = new Object();

    private boolean mIsBind = false;

    private SenseWrapper(Context context) {
        if (context != null) {
            mContext = context;
        }
    }

    public static SenseWrapper getInstance(Context context) {
        if (sInstance == null && context != null) {
            sInstance = new SenseWrapper(context);
        }
        return sInstance;
    }
    ...
    public int authenticate(byte[] token, int width, int height) {
        int authenticate;
        synchronized (mLock) {
            authenticate = -1001;
            try {
                if (!mIsBind) {
                    ...
                    authenticate = mService.authenticate(token, width, height);
                } else {
