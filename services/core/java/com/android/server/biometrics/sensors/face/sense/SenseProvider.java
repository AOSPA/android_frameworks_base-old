...
public class SenseProvider implements ServiceProvider {
    ...
    private static final String SERVICE_NAME = "co.aospa.sense.Service";
    ...
    public static class HalResultController extends ISenseServiceReceiver.Stub {
        ...
        @Override
        public void onAuthenticated(int faceId, byte[] token) {
            mHandler.post(() -> {
                final BaseClientMonitor client = mScheduler.getCurrentClient();
            ...
            mHandler.post(() -> {
                ...
                if (removed.length > 0) {
                    ...
                    for (int i = 0; i < removed.length; i++) {
                        final int id = removed[i];
                        final Face face = new Face("", id, Long.valueOf(DEVICE_ID));
                        final int remaining = (removed.length - i) - 1;
        ...
        @Override
        public void onEnumerate(int[] faceIds) {
            mHandler.post(() -> {
                final BaseClientMonitor client = mScheduler.getCurrentClient();
    ...
    @VisibleForTesting
    SenseProvider(@NonNull Context context,
	...
	public SenseProvider(@NonNull Context context, 
	    @NonNull FaceSensorPropertiesInternal sensorProps, 
		@NonNull LockoutResetDispatcher lockoutResetDispatcher) {
        this(context, sensorProps, lockoutResetDispatcher, new Handler(Looper.getMainLooper(), new BiometricScheduler(TAG, 0, null), BiometricContext.getInstance(context));
    }
