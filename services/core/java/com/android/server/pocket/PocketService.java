...
public class PocketService extends SystemService implements IBinder.DeathRecipient {
     ...
     */
    private static final float POCKET_LIGHT_MAX_THRESHOLD = 1.0f;
    ...
    private void dispatchCallbacksIfNeeded() {
        ...
        if (isDeviceInPocket != wasDeviceInPocket) {
            if (mInteractive) {
                if (wasDeviceInPocket) {
                    mHandler.sendEmptyMessageDelayed(PocketHandler.MSG_UNREGISTER_TIMEOUT, 1000);
                } else {
                    mHandler.removeMessages(PocketHandler.MSG_UNREGISTER_TIMEOUT);
                }
            }
