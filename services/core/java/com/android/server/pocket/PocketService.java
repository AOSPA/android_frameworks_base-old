...
public class PocketService extends SystemService implements IBinder.DeathRecipient {
    ...
    private static final boolean DEBUG_SPEW = PocketConstants.DEBUG_SPEW;
    ...
    private void handleLightSensorEvent(SensorEvent sensorEvent) {
        ...
        try {
            if (sensorEvent == null) {
                ...
                final boolean isPoor = value >= 0.0f /* absent */ && value <= 1.0f /* maximum light observed in pocket */;
    ...
    private void SPEW() {
        if (DEBUG_SPEW) {
            try {
                Log.d(TAG, "{SPEW}: Device in pocket ? " + isDeviceInPocket()
