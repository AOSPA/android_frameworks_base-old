...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    // Custom policy supported key states.
    ...
    private void handleLongPressOnKeyCode(int keyCode) {
        int longPressBehavior = getLongPressBehavior(keyCode);
    ...
    private void handleDoubleTapOnKeyCode(int keyCode) {
        int doubleTapBehavior = getDoubleTapBehavior(keyCode);
    ...
    private Runnable createDoubleTapTimeoutRunnable(final int keyCode) {
        Runnable r = new Runnable() {
            @Override
            public void run() {
                final boolean doubleTapPending = isDoubleTapPending(keyCode);
                if (doubleTapPending) {
                    setDoubleTapPending(keyCode, false);
    ...
    private void readConfigurationDependentBehaviors() {
        ...
        mLongPressOnHardwareHomeBehavior = res.getInteger(
        ...
        if (mLongPressOnHardwareHomeBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHardwareHomeBehavior > KEY_ACTION_LAST_APP) {
            mLongPressOnHardwareHomeBehavior = KEY_ACTION_NOTHING;
        }

        mDoubleTapOnHardwareHomeBehavior = res.getInteger(
        ...
        if (mDoubleTapOnHardwareHomeBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHardwareHomeBehavior > KEY_ACTION_LAST_APP) {
            mDoubleTapOnHardwareHomeBehavior = KEY_ACTION_NOTHING;
        }

        mLongPressOnHardwareMenuBehavior = res.getInteger(
        ...
        if (mLongPressOnHardwareMenuBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHardwareMenuBehavior > KEY_ACTION_LAST_APP) {
            mLongPressOnHardwareMenuBehavior = KEY_ACTION_NOTHING;
        }

        mDoubleTapOnHardwareMenuBehavior = res.getInteger(
        ...
        if (mDoubleTapOnHardwareMenuBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHardwareMenuBehavior > KEY_ACTION_LAST_APP) {
            mDoubleTapOnHardwareMenuBehavior = KEY_ACTION_NOTHING;
        }

        mLongPressOnHardwareBackBehavior = res.getInteger(
        ...
        if (mLongPressOnHardwareBackBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHardwareBackBehavior > KEY_ACTION_LAST_APP) {
            mLongPressOnHardwareBackBehavior = KEY_ACTION_NOTHING;
        }

        mDoubleTapOnHardwareBackBehavior = res.getInteger(
        ...
        if (mDoubleTapOnHardwareBackBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHardwareBackBehavior > KEY_ACTION_LAST_APP) {
            mDoubleTapOnHardwareBackBehavior = KEY_ACTION_NOTHING;
        }

        mLongPressOnHardwareAssistBehavior = res.getInteger(
        ...
        if (mLongPressOnHardwareAssistBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHardwareAssistBehavior > KEY_ACTION_LAST_APP) {
            mLongPressOnHardwareAssistBehavior = KEY_ACTION_NOTHING;
        }

        mDoubleTapOnHardwareAssistBehavior = res.getInteger(
        ...
        if (mDoubleTapOnHardwareAssistBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHardwareAssistBehavior > KEY_ACTION_LAST_APP) {
            mDoubleTapOnHardwareAssistBehavior = KEY_ACTION_NOTHING;
        }

        mLongPressOnHardwareAppSwitchBehavior = res.getInteger(
        ...
        if (mLongPressOnHardwareAppSwitchBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHardwareAppSwitchBehavior > KEY_ACTION_LAST_APP) {
            mLongPressOnHardwareAppSwitchBehavior = KEY_ACTION_NOTHING;
        }

        mDoubleTapOnHardwareAppSwitchBehavior = res.getInteger(
        ...
        if (mDoubleTapOnHardwareAppSwitchBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHardwareAppSwitchBehavior > KEY_ACTION_LAST_APP) {
            mDoubleTapOnHardwareAppSwitchBehavior = KEY_ACTION_NOTHING;
        }

        mLongPressOnHardwareCameraBehavior = res.getInteger(
        ...
        if (mLongPressOnHardwareCameraBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHardwareCameraBehavior > KEY_ACTION_LAST_APP) {
            mLongPressOnHardwareCameraBehavior = KEY_ACTION_NOTHING;
        }

        mDoubleTapOnHardwareCameraBehavior = res.getInteger(
        ...
        if (mDoubleTapOnHardwareCameraBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHardwareCameraBehavior > KEY_ACTION_LAST_APP) {
     ...
     */
    static final int[] SUPPORTED_KEY_CODES = {
     ...
     */
    private boolean isConsumed(int keyCode) {
     ...
     * @param keyCode the KeyEvent key code.
     */
    private void setConsumed(int keyCode, boolean consumed) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_HOME:
     ...
     * @param keyCode the KeyEvent key code.
     */
    private void setPressed(int keyCode, boolean pressed) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_HOME:
     ...
     * @param keyCode the KeyEvent key code.
     */
    private void setDoubleTapPending(int keyCode, boolean pending) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_HOME:
     ...
     * @param keyCode the KeyEvent key code.
     */
    private int getDoubleTapBehavior(int keyCode) {
        int behavior = -1;
        switch (keyCode) {
            case KeyEvent.KEYCODE_HOME:
     ...
     * @param keyCode the KeyEvent key code.
     */
    private int getLongPressBehavior(int keyCode) {
        int behavior = -1;
        switch (keyCode) {
            case KeyEvent.KEYCODE_HOME:
     ...
     * @param keyCode the KeyEvent key code.
     */
    private boolean isDoubleTapPending(int keyCode) {
    ...
    private boolean isSupported(int keyCode) {
        ...
        int length = SUPPORTED_KEY_CODES.length;
        for (int i = 0; i < length; i++) {
            if (SUPPORTED_KEY_CODES[i] == keyCode) {
                supported = true;
                break;
            }
        }
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (isSupported(keyCode) && !isCustomSource) {
            ...
            if (!down) {
                ...
                if (isConsumed(keyCode)) {
                    setConsumed(keyCode, false);
                    return -1;
                }
            ...
            if (repeatCount == 0) {
                ...
                setConsumed(keyCode, false);
                final boolean doubleTapPending = isDoubleTapPending(keyCode);
                final int longPressBehavior = getLongPressBehavior(keyCode);
                final int doubleTapBehavior = getDoubleTapBehavior(keyCode);
                if (doubleTapPending) {
                    setDoubleTapPending(keyCode, false);
                    mHandler.removeCallbacks(doubleTapRunnable);
                    setConsumed(keyCode, true);
                    if (!keyguardOn) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        if (DEBUG_INPUT) {
            Log.d(TAG, "interceptKeyBeforeQueueing(): event =" + event.toString()
                    ...
                    + ", virtualKey = " + virtualKey + ", virtualHardKey = " + virtualHardKey
                    + ", navBarKey = " + navBarKey + ", fromSystem = " + fromSystem
                    + ", supported = " + isSupported(keyCode));
        }

        // Apply custom policy for supported key codes.
        if (isSupported(keyCode) && !isCustomSource) {
            if (mNavBarEnabled && !navBarKey /* TODO> && !isADBVirtualKeyOrAnyOtherKeyThatWeNeedToHandleAKAWhenMonkeyTest!!! */) {
                if (DEBUG_INPUT) {
