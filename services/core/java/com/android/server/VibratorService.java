...
public class VibratorService extends IVibratorService.Stub
    ...
    private static final long ONEPLUS_EFFECT_THUD = 4509003;
    ...
    private long doOneplusEncoding(long millis, AudioAttributes attrs){
        ...
        if(eff instanceof VibrationEffect.Prebaked) {
            switch((((VibrationEffect.Prebaked) eff)).getId()){
                case VibrationEffect.EFFECT_CLICK:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_CLICK");
                    return ONEPLUS_EFFECT_CLICK;
                case VibrationEffect.EFFECT_DOUBLE_CLICK:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_DOUBLE_CLICK");
                    return ONEPLUS_EFFECT_DOUBLE_CLICK;
                case VibrationEffect.EFFECT_HEAVY_CLICK:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_HEAVY_CLICK");
                    return ONEPLUS_EFFECT_HEAVY_CLICK;
                case VibrationEffect.EFFECT_TEXTURE_TICK:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_TEXTURE_TICK");
                    return ONEPLUS_EFFECT_TEXTURE_TICK;
                case VibrationEffect.EFFECT_TICK:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_TICK");
                    return ONEPLUS_EFFECT_TICK;
                case VibrationEffect.EFFECT_POP:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_POP");
                    return ONEPLUS_EFFECT_POP;
                case VibrationEffect.EFFECT_THUD:
                    Slog.w(TAG, "doOneplusEncoding() EFFECT_THUD");
            ...
            Slog.w(TAG, "doOneplusEncoding() ELSE");
    ...
    private void doVibratorOn(long millis, int amplitude, int uid, AudioAttributes attrs) {
        ...
        try {
            ...
            if(millis == 0){
                Slog.e(TAG, "doOneplusEncoding() This shouldn't have happened." +
                    " Send logs to @rituj_b");
                return;
            }
            synchronized (mInputDeviceVibrators) {
