...
public class KeyguardPinViewController
    ...
    protected void onViewAttached() {
        ...
        mPasswordEntry.setQuickUnlockListener(new QuickUnlockListener() {
            public void onValidateQuickUnlock(String password) {
                if (password != null && password.length() == mLockPatternUtils.getPINPasswordLength(mCurrentUserId)) {
    ...
    private AsyncTask<?, ?, ?> validateQuickUnlock(final String password) {
        AsyncTask<Void, Void, Boolean> task = new AsyncTask<Void, Void, Boolean>() {

            @Override
            protected Boolean doInBackground(Void... args) {
                try {
                    return mLockPatternUtils.checkCredential(LockscreenCredential.createPinOrNone(password),
                            mCurrentUserId, null);
