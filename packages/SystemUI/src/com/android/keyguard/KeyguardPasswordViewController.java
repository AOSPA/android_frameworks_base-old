...
public class KeyguardPasswordViewController
    ...
    private final TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (!TextUtils.isEmpty(s)) {
                ...
                if (entry.size() == getPINPasswordLength()) {
                    validateQuickUnlock(mLockPatternUtils, entry, mCurrentUserId);
                }
            }
        }
    };
    ...
    private AsyncTask<?, ?, ?> validateQuickUnlock(final LockPatternUtils utils,
            final LockscreenCredential password, final int userId) {
    ...
    private void runQuickUnlock(Boolean matched) {
        if (matched) {
            ...
            mKeyguardSecurityCallback.dismiss(true, mCuurentUserId);
    ...
    private int getPINPasswordLength() {
        ...
        try {
            mPINPasswordLength = (int) mLockPatternUtils.getLockSettings().getLong(
                    "lockscreen.pin_password_length", -1, mCurrentUser);
        } catch (Exception e) {
            // do nothing
        }
