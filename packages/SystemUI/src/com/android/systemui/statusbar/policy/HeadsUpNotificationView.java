...
public class HeadsUpNotificationView extends FrameLayout implements SwipeHelper.Callback,
    ...
    public boolean showNotification(NotificationData.Entry headsUp) {
        if (mBar.isExpandedVisible()) {
            if (mHeadsUp != null) {
                release(); // first current one
                release(headsUp); // then new one
                mBar.scheduleHeadsUpClose();
            } else {
                // not showing, add the inflated view back to the shade
                releaseAndClose(headsUp);
            }
        ...
        if (mContentHolder != null) {
            ...
            // too soon! Reparent entries and exit here.
            if (mHeadsUp != null) release();
            if (headsUp != null) release(headsUp);
            mBar.scheduleHeadsUpClose();
            return false;
        }
    ...
    public void clear(NotificationData.Entry headsUp) {
        headsUp = null;
        mBar.scheduleHeadsUpClose();
    }
    ...
    public void release(NotificationData.Entry headsUp) {
        if (headsUp != null) {
            mBar.displayNotificationFromHeadsUp(headsUp.notification);
            headsUp = null;
        }
    }

    public void releaseAndClose() {
        releaseAndClose(mHeadsUp);
    }

    public void releaseAndClose(NotificationData.Entry headsUp) {
        release(headsUp);
        mBar.scheduleHeadsUpClose();
    }
