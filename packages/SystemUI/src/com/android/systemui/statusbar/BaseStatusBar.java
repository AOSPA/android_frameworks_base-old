...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    public ArrayList<StatusBarNotification> getSbn() {
        ArrayList<StatusBarNotification> key = new ArrayList<StatusBarNotification>();
        int maxIcons = mContext.getResources().getInteger(R.integer.pie_maximum_notifications);
        for (Entry entry : mNotificationData.getActiveNotifications()) {
            StatusBarNotification notif = entry.notification;
            key.add(notif);
        }
        if (key.size() > maxIcons) {
            key.subList(maxIcons, key.size()).clear();
        }
        return key;
    }

    public ArrayList<Pair<String, Icon> > getNotificationIcons() {
        ArrayList<Pair<String, Icon>> notifications = new ArrayList<Pair<String, Icon>>();
        for (Entry entry : mNotificationData.getActiveNotifications()) {
            String pkgName = entry.notification.getPackageName();
            Icon icon = entry.notification.getNotification().getSmallIcon();
            notifications.add(new Pair<String, Icon>(pkgName, icon));
        }
    ...
    public void performRemoveNotificationExt(StatusBarNotification n, boolean removeView) {
        performRemoveNotification(n, removeView);
    }
