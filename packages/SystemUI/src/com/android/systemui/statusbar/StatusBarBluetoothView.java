...
public class StatusBarBluetoothView extends FrameLayout implements StatusIconDisplayable {
    ...
    private void updateBatteryIcon(int batteryLevel) {
        ...
        if (batteryLevel >= 0 && batteryLevel <= 100) {
            String resName = "ic_bluetooth_battery_" + batteryLevel / 10;
            int resId = mContext.getResources().getIdentifier(resName, "drawable",
                    mContext.getPackageName());
            if (resId == 0) {
                Log.e(TAG, "updateBatteryIcon: unable to get drawable, batteryLevel=" + batteryLevel
                        + " resName" + resName);
