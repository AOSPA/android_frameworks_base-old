...
public class PieController extends EdgeGestureManager.EdgeGestureActivationListener implements OnTouchListener, OnClickListener {
    ...
    private boolean showPie() {
        final boolean pieEnabled = Settings.Secure.getInt(
                mContext.getContentResolver(),
                Settings.Secure.PIE_STATE, 0) == 1;
        final int immersiveMode = Settings.Secure.getInt(
                mContext.getContentResolver(),
                Settings.Secure.SYSTEM_DESIGN_FLAGS, 0);
        final int IMMERSIVE_OFF = 0;
        return pieEnabled && immersiveMode !=
                View.SYSTEM_DESIGN_FLAG_IMMERSIVE_STATUS
                && immersiveMode != IMMERSIVE_OFF;
    }

    private void attachPie(int gravity) {
        if (showPie()) {
            // want some slice?
            switch (gravity) {
