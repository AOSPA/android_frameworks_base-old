...
public class PieMenu extends RelativeLayout {
    ...
    // Animators
    private AnimatorSet mGrowAnimators;
    ...
    private void getDimensions() {
        ...
        if (mToggleGrowAnimator != null) {
            animList.add(mToggleGrowAnimator);
        }

        // outside snappoint animator
        ...
        mToggleOutsideAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                if (mToggleGrowAnimator == null) return;
                final Handler mHandler = new Handler();
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        if (animation.getAnimatedFraction() < 1f) {
                            mToggleGrowAnimator.setCurrentFraction(
        ...
        mToggleOutsideAnimator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                mToggleOutsideAnimator.removeAllUpdateListeners();
                switchSnapPoints();
            }
        });
        if (mToggleOutsideAnimator != null) {
            animList.add(mToggleOutsideAnimator);
        }

        // Animatorset for grow animators
        mGrowAnimators = new AnimatorSet();
        mGrowAnimators.playSequentially(animList);
    ...
    private void switchSnapPoints() {
        if (mGrowAnimators != null && !mGrowAnimators.isRunning()) {
            // Animators not running, return
            return;
        }
    ...
    private void layoutPie() {
        ...
        for (PieItem item : mItems) {
            ...
            sweep = ((float) (Math.PI - 2 * 0f) /
                    itemCount) * (item.isLesser() ? 0.65f : 1 + adjustedSweep);
            angle = (0f + sweep / 2 - (float) Math.PI / 2);
        ...
        if (mOpen) {
            ...
            for (int i = 0; i < mNumberOfTogglePoints; i++) {
                ...
                float fraction = 1f + (toggle.active ?
                        toggleGrowAnimatorFraction * (isNotPoint ? 0.4f : 0.8f) : 0f);
                float toggleOutSidefraction = 1f + (toggle.active ?
                        toggleOutSideAnimatorFraction * (isNotPoint ? 0.7f : 1.1f) : 0);
                toggle.draw(canvas, mToggleBackground, fraction, backgroundFraction);
                // Only draw when outside animator is running, otherwise we are drawing twice,
                // which shouldn't be happening
                if (mToggleOutsideAnimator.isStarted()) {
