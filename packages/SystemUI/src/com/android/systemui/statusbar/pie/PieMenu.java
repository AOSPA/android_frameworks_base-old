...
public class PieMenu extends RelativeLayout {
    ...
    private void getDimensions() {
        ...
        if (mPieHelper.isAssistantAvailable()) {
            for (PieItem item : mItems) {
                item.setColor(mNOTLogo, mBackgroundColor);
            }
        ...
        // background animator
        mBackgroundAnimator = ValueAnimator.ofFloat(0.0f, 1.0f);
        mBackgroundAnimator.setDuration(500);
        ...
        mToggleGrowAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                if (animation.getAnimatedFraction() >= 0.95 && mToggleOuterGrowAnimator != null
    ...
    protected void show(boolean show) {
        ...
        if (mOpen) {
            ...
            // give the animation some time to run before calculating pie
            final Handler mHandler = new Handler();
            mHandler.postDelayed(layoutPie, 150);
        }
        invalidate();
    }

    private Runnable layoutPie = new Runnable() {
        @Override
        public void run() {
            layoutPie();
        }
    ...
    private void layoutPie() {
        ...
        for (PieItem item : mItems) {
            ...
            item.setGeometry(itemStart, sweep);
            total += sweep;
        }
    }
    ...
    private void animateInRest() {
        ...
        mHasShown = true;
        mBackgroundAnimator.setStartDelay(275);
    ...
    protected void onDraw(final Canvas canvas) {
        if (mOpen) {
            ...
            if (isAllowedToDraw()) {
                ...
                mToggleOuterBackground.setAlpha(100);
            ...
            for (int i = 0; i < mNumberOfTogglePoints; i++) {
                ...
                float fraction = 1f + (toggle.active ?
                        toggleGrowAnimatorFraction * (isNotPoint ? 0.45f : 0.8f) : 0f);
                float toggleOutSidefraction = (toggle.active ? 1f + toggleOuterAnimatorFraction
                        * (isNotPoint ? 0.92f : 1.1f) : 0);
                ...
                if (mToggleOuterGrowAnimator.isStarted()) {
                    toggle.draw(canvas, mToggleOuterBackground, toggleOutSidefraction,
                            toggleOuterAnimatorFraction);
                }
            }
    ...
    private void drawItem(Canvas canvas, PieItem item, float fraction) {
        if (item.getView() != null) {
            ...
            item.setColor(view, mBackgroundColor);
    ...
    public boolean onTouchEvent(MotionEvent evt) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // take back snap points if user decides to abandon his gesture
