...
public class PieMenu extends RelativeLayout {
    ...
    private void getDimensions() {
        ...
        mBackgroundAnimator.setDuration((int) (OVERALL_SPEED * 1.5));
        mBackgroundAnimator.setInterpolator(new DecelerateInterpolator());

        mToggleOutsideAnimator = ValueAnimator.ofInt(0, 1);
        mToggleOutsideAnimator.setDuration((int) (OVERALL_SPEED - 85));
        ...
        mToggleGrowAnimator.setDuration((int) (OVERALL_SPEED + 85));
        ...
        if (mOpen) {
            ...
            for (int i = 0; i < mNumberOfTogglePoints; i++) {
                ...
                float fraction = 1f + (toggle.active ?
                        mToggleGrowAnimator.getAnimatedFraction() * 2f : 0f);
                toggle.draw(canvas, mToggleBackground, fraction, backgroundFraction);
                if (mToggleOutsideAnimator.isRunning()) {
                    fraction = 1f + (toggle.active ?
                            mToggleOutsideAnimator.getAnimatedFraction() * 2.3f : 0f);
                    toggle.draw(canvas, mToggleExtraBackground, fraction, backgroundFraction);
                }
            }
