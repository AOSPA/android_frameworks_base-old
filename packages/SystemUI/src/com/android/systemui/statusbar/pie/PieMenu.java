    ...
    protected void updateNotifications(boolean update) {
        ...
        if (isAllowedToDraw()) {
            ...
            for (Pair<String, Icon> icon : mIcons) {
                ...
                if (update) {
                    // Invalidate only the current view,
                    // since invalidate calls are expensive
                    view.invalidate();
                }
            }
        }
        ...
        if (isAllowedToDraw()) {
            ...
            if (mPieHelper.getNotificationCount() > 0) {
                canvas.drawLine(mWidth / (isLandScape() ? 3 : 2) - mLineLength / 2, (mPieBottom
                        ...
                        mWidth / (isLandScape() ? 3 : 2) + mLineLength / 2, (mPieBottom ? mHeight / 2 - mLineOffset
            ...
            case MotionEvent.ACTION_UP:
                if (mOpen) {
                    ...
                    if (item != null && item.getView() != null
                            ...
                            && item.isSelected()) {
                        if (mHapticFeedback && item.getName().equals(PieController.RECENT_BUTTON)) {
