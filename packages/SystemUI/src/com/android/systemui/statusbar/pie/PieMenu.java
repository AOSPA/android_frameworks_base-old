...
public class PieMenu extends RelativeLayout {
    ...
    public PieMenu(Context context, PieControlPanel panel) {
        ...
        // pie circles
        // drop shadow doesn't support hardware acceleration so we can't use it here
    ...
    private void getDimensions() {
        ...
        // 2nd/outside snappoint animator
        mToggleOutsideAnimator = ValueAnimator.ofInt(0, 2);
        ...
        mToggleOutsideAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                if (mToggleOutsideAnimator.getAnimatedFraction() == 1) {
                    mToggleGrowAnimator.reverse();
                    // If somehow the animator did not start
                    // try to start it again to make sure it runs.
                    if (!mToggleGrowAnimator.isRunning()) {
                        mToggleGrowAnimator.reverse();
                    }
                }
            }
        ...
        mToggleOutsideAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                // Prevent switching snappoints when animation has been cancelled
                // This can happen when letting the animation run for a certain time which
                // Is less than the full animation time, then cancelling the animation.
                // Without this check it will still execute switchSnapPoints(). Fix it.
                if (mToggleGrowAnimator.isRunning()) {
                    switchSnapPoints();
                }
            }
    ...
    private void switchSnapPoints() {
        // mToggleGrowAnimator anim speed - mToggleOutsideAnimator anim speed(385-250) + 10 = 145
        final int animTimeout = 145;
        final Handler mHandler = new Handler();
        // "Hack" to wait until mToggleGrowAnimator has fully reversed before dismissing
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (mOpen) {
            ...
            for (int i = 0; i < mNumberOfTogglePoints; i++) {
                ...
                float toggleGrowAnimatorFraction = mToggleGrowAnimator.getAnimatedFraction();
                float toggleOutSideAnimatorFraction = mToggleOutsideAnimator.getAnimatedFraction();
                float fraction = 1f + (toggle.active ?
                        toggleGrowAnimatorFraction * 0.5f : 0f);
                float toggleOutSidefraction = 1f + (toggle.active ?
                        toggleOutSideAnimatorFraction * 0.8f : 0);
                toggle.draw(canvas, mToggleBackground, fraction, backgroundFraction);
                // I know this is silly, but for some reason weird things happen
                // with NOT icon without this check :(
                if (mToggleOutsideAnimator.isRunning()) {
                    toggle.draw(canvas, toggle.active ? mToggleExtraBackground
                            : mToggleBackground, toggleOutSidefraction,
                            toggleOutSideAnimatorFraction);
                }
            }
    ...
    private boolean isAllowedToDraw() {
        ...
        return mPanel.isKeyguardSecure() && immersiveModeAllowsPie !=
