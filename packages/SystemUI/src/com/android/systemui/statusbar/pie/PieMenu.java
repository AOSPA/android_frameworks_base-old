...
public class PieMenu extends RelativeLayout {
    ...
    private void getDimensions() {
        ...
        mBackgroundAnimator.addUpdateListener(new AnimatorUpdateListener(mBackgroundAnimator));
    ...
    private void animateInStartup() {
        ...
        mPieFadeAnimator.setStartDelay(10);
    ...
    private void drawItem(Canvas canvas, PieItem item, float fraction) {
        ...
        canvas.restoreToCount(state);
        if (item.isSelected()) {
            final int mRippleRadius = 80;
            mRipplePaint.setAlpha(50);
            // Draw the ripple circle
            // TODO: Figure out why X is wrong (#BlameGoogle?)
            canvas.drawCircle(x, y, mRippleRadius, mRipplePaint);
        }
    }
    ...
    private class AnimatorUpdateListener implements ValueAnimator.AnimatorUpdateListener {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            if (animationIndex == mBackgroundAnimator) {
                mBackgroundFraction = animation.getAnimatedFraction();
            }

            if (animationIndex == mToggleGrowAnimator && animation.getAnimatedFraction()
                    >= 0.95 && mToggleOuterGrowAnimator != null
