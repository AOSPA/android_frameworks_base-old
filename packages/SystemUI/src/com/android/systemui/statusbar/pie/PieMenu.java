...
public class PieMenu extends RelativeLayout {
    ...
    private static final int ANIMATOR_PIE_MOVE = 0;
    private static final int ANIMATOR_PIE_GROW = 1;
    private static final int ANIMATOR_BACKGROUND = 2;
    private static final int ANIMATOR_TOGGLE_GROW = 3;
    ...
    private final CustomValueAnimator[] mAnimators = new CustomValueAnimator[ANIMATOR_TOGGLE_GROW + 1];
    ...
    private void getDimensions() {
        ...
        // snap
        int mSnapRadius = mResources.getDimensionPixelSize(R.dimen.pie_snap_radius);
        int mSnapOffset = mResources.getDimensionPixelSize(R.dimen.pie_snap_offset);
        ...
        for (Pair<String, Icon> icon : mIcons) {
            ...
            switch (mPanelOrientation) {
                case Gravity.LEFT:
                case Gravity.RIGHT:
                    lp.topMargin = mHeight / 4 + (int) iconOffsetYside;
                    break;
                case Gravity.BOTTOM:
                    lp.addRule(RelativeLayout.ALIGN_PARENT_TOP);
                    lp.topMargin = mHeight / 2 + (int) iconOffsetY;
                    break;
            }
        ...
        // move circles in
        mAnimators[ANIMATOR_PIE_MOVE].duration = OVERALL_SPEED;
    ...
    private void cancelAnimation() {
        for (CustomValueAnimator mAnimator : mAnimators) {
            mAnimator.cancel();
            invalidate();
        }
    }
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (mOpen) {
            ...
            // draw background
            final float backgroundFraction = mAnimators[ANIMATOR_BACKGROUND].fraction;
            ...
            canvas.drawLine(mWidth / 2 - mLineLength / 2,
                    mHeight / (pieBottom ? 2 : 4) +
                            (pieBottom ? mLineOffset : mLineOffsetSide),
            ...
            for (int i = 0; i < mNumberOfTogglePoints; i++) {
                ...
                final float fraction = 1f + (toggle.active ?
                        mAnimators[ANIMATOR_TOGGLE_GROW].fraction * 2f : 0f);
                toggle.draw(canvas, mToggleBackground, fraction, backgroundFraction);
            }

            final float pieMoveFraction = mAnimators[ANIMATOR_PIE_MOVE].fraction;
            final float pieGrowFraction = mAnimators[ANIMATOR_PIE_GROW].fraction;
    ...
    private boolean isKeyguardUnsecure() {
    ...
    public boolean onTouchEvent(MotionEvent evt) {
        ...
        if (MotionEvent.ACTION_DOWN == action) {
            ...
            for (int i = 0; i < mNumberOfTogglePoints; i++) {
                ...
                if (toggleDistance < toggle.radius && isKeyguardUnsecure()) {
                    if (!toggle.active) {
            ...
            // trigger the shades
            if (mCenterDistance > shadeThreshold && isKeyguardUnsecure()) {
    ...
    private class CustomValueAnimator {
        ...
        CustomValueAnimator(int animateIndex) {
            index = animateIndex;
            manual = false;
