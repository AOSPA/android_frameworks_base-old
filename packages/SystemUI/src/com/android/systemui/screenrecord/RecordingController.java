...
public class RecordingController implements CallbackController<RecordingStateChangeCallback> {
    ...
    public void startCountdown(long millisInFuture, long countDownInterval, PendingIntent startIntent, PendingIntent stopIntent) {
        ...
        CountDownTimer timer = new CountDownTimer(millisInFuture, countDownInterval) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                while (it.hasNext()) {
                    ((RecordingStateChangeCallback) it.next()).onCountdown(millisUntilFinished);
                }
            }
    ...
    public void cancelCountdown() {
        ...
        mIsStarting = false;
        Iterator<RecordingStateChangeCallback> it = mListeners.iterator();
        while (it.hasNext()) {
    ...
    public void updateState(boolean isRecording) {
        ...
        Iterator<RecordingStateChangeCallback> it = mListeners.iterator();
        while (it.hasNext()) {
            RecordingStateChangeCallback cb = it.next();
            if (isRecording) {
                cb.onRecordingStart();
            } else {
                cb.onRecordingEnd();
    ...
    public void addCallback(RecordingStateChangeCallback cb) {
        mListeners.add(cb);
    }

    public void removeCallback(RecordingStateChangeCallback cb) {
