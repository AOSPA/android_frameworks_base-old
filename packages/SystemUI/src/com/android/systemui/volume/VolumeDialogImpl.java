...
public class VolumeDialogImpl implements VolumeDialog,
        ...
        if (mSettingsView != null) {
            mSettingsView.setVisibility(
                    mDeviceProvisionedController.isCurrentUserSetup() &&
                            ...
                            VISIBLE : GONE);
            mSettingsView.findViewById(R.id.divider).setVisibility(GONE);
        }
    ...
    private void updateRowsH(final VolumeRow activeRow) {
        ...
        for (final VolumeRow row : mRows) {
            ...
            final ViewGroup.MarginLayoutParams params = (ViewGroup.MarginLayoutParams) sliderFrame.getLayoutParams();
            if (shouldBeVisible) {
                visibleRows++;
                if (visibleRows == 1) {
    ...
    private void updateVolumeRowTintH(VolumeRow row, boolean isActive) {
        ...
        row.icon.setImageTintList((mDarkMode) ? Utils.getColorAttr(mContext, android.R.attr.colorForeground).withAlpha(255) : Utils.getColorAccent(mContext).withAlpha(255));
        row.header.setTextColor((mDarkMode) ? Utils.getColorAttr(mContext, android.R.attr.colorForeground).withAlpha(255) : Utils.getColorAccent(mContext).withAlpha(255));
        row.cachedTint = tint;
    }
    ...
    private final class VolumeSeekBarChangeListener implements OnSeekBarChangeListener {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if (!fromUser) return;
            Util.setText(mRow.header, (int) ((float) mRow.ss.level/mRow.ss.levelMax * 100) + "%");
