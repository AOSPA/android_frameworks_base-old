...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private void cleanExpandedRows() {
        ...
        if (isMediaButtonVisible && !mODIServiceComponentEnabled) {
            ...
            Log.d(TAG, "odienabled = false, mediavisible = true");
        } else if (mODIServiceComponentEnabled) {
            float zMedia = z;
            Log.d(TAG, "odienabled = true");
            float widthMedia = width;
            if (isMediaButtonVisible) {
                Log.d(TAG, "mediavisible = true");
                animateViewOut(mMediaButtonView, false, widthMedia, zMedia);
                widthMedia += widthMedia;
                zMedia /= 2;
            }
            animateViewOut(mODICaptionsView, false, widthMedia, zMedia);
            hideCaptionsTooltip();
        }
            ...
            mExpandRows.setOnClickListener(v -> {
                if (!mExpanded) {
                    ...
                    if (showMediaOutput && !mODIServiceComponentEnabled) {
                        ...
                        float widthMedia = width;
                        if (showMediaOutput) {
                            animateViewIn(mMediaButtonView, false, widthMedia, zMedia);
                            widthMedia += widthMedia;
                            zMedia /= 2;
                        }
                        animateViewIn(mODICaptionsView, false, widthMedia, zMedia);
