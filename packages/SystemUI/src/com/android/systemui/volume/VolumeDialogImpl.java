...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private final HandlerThread mThreadSecondary = new HandlerThread("SecondaryThread");
    private final H mHandlerSecondary;
    ...
    public VolumeDialogImpl(Context context) {
        ...
        if (mVibrateOnSlider) {
            mThreadSecondary.start();
            mHandlerSecondary = new H(mThreadSecondary.getLooper());
        } else {
            mHandlerSecondary = null;
        }
        setDarkMode();
    }
    ...
    private void initDialog() {
        ...
        View dialogContainer = mDialog.findViewById(R.id.volume_dialog_container);
        dialogContainer.setLayoutDirection(
                isAudioPanelOnLeftSide() ? View.LAYOUT_DIRECTION_RTL : View.LAYOUT_DIRECTION_LTR);
        dialogContainer.setOnTouchListener((v, event) -> {
        ...
        Log.d(TAG, "initDialog mMediaOutputView.getChildCount():" + mMediaOutputView.getChildCount());
    ...
    private void cleanExpandedRows() {
        VolumeRow ring = findRow(AudioManager.STREAM_RING);
        VolumeRow alarm = findRow(AudioManager.STREAM_ALARM);
        VolumeRow media = findRow(AudioManager.STREAM_MUSIC);
        VolumeRow active = getActiveRow();
        float width = (float) media.view.getWidth() + (float) mContext
                .getResources().getDimension(R.dimen.volume_dialog_row_spacer);
        width = (isAudioPanelOnLeftSide() ? 1f : -1f) * width;
        animateView(mMediaOutputScrollView, false, width, mElevation, false);
        if (alarm != null) {
            animateView(alarm.view, false, 0, mElevation, active == alarm);
            width = (alarm == active) ? width/2 : width;
        }
        if (ring != null) {
            animateView(ring.view, false, width, mElevation/2, active == ring);
            width = (ring == active) ? width : width * 2;
        }
        animateView(media.view, false, width, 0, true);
        setSelectedMediaRows(false);
    }
    ...
    private void initRow(final VolumeRow row, final int stream, int iconRes, int iconMuteRes,
        ...
        row.slider.setProgressTintList(Utils.getColorAccent(mContext)
                .withAlpha(mDarkMode ?
            ...
            mExpandRows.setOnClickListener(v -> {
                if (!mExpanded) {
                    ...
                    if (mTelephonyManager.getCallState() == TelephonyManager.CALL_STATE_IDLE) {
                        Util.setVisOrGone(mMediaOutputScrollView, true);
                        animateView(mMediaOutputScrollView, true, width*2, mElevation, isLandscape());
                    }
                    if (alarm != null) {
                        Util.setVisOrGone(alarm.view, true);
                        animateView(alarm.view, true, 0, mElevation, alarm == active);
                        width = (alarm == active) ? width/2 : width;
                    }
                    if (ring != null) {
                        Util.setVisOrGone(ring.view, true);
                        animateView(ring.view, true, width, mElevation/5, ring == active);
                        width = (ring == active) ? width : width * 2;
                    }
                    Util.setVisOrGone(media.view, true);
                    animateView(media.view, true, width, 0, true);
                    setSelectedMediaRows(true);
                    mExpanded = true;
                } else {
                    cleanExpandedRows();
                    mExpanded = false;
    ...
    private void animateView(View view, boolean slideIn, float negX, float startZ, boolean viewStays) {
        if (view == null) {
            return;
        }
        if (slideIn) {
            view.setTranslationX(-negX);
            view.setTranslationZ(viewStays ? 0 : -startZ);
            view.setAlpha(viewStays ? 1 : 0);
            view.animate()
                .alpha(1)
                .translationX(0)
                .translationZ(0)
            ...
            view.animate()
                .alpha(viewStays ? 1 : 0)
                .translationX(-negX)
                .translationZ(viewStays ? 0 : -startZ)
                .setDuration(DIALOG_SHOW_ANIMATION_DURATION)
                .setInterpolator(new SystemUIInterpolators.LogDecelerateInterpolator())
                .withEndAction(() -> {
                    Util.setVisOrGone(view, viewStays);
                    view.setTranslationX(0);
                    view.setTranslationZ(0);
                    view.setAlpha(1);
                })
    ...
    public void onSelectedDeviceStateChanged(MediaDevice device, int state) {
        Log.d(TAG, "onSelectedDeviceStateChanged " + device.getName() + " state=" + state);
    }

    private void updateMediaOutputView() {
        final int N = mMediaDevices.size();
        if (N < mMediaOutputView.getChildCount()) {
        ...
        for (MediaDevice device : mMediaDevices) {
            ...
            MediaOutputRow row = findMediaOutputRow(device);
            if (device.isConnected()) {
                if (row == null) {
                    row = new MediaOutputRow();
                    row.device = device;
                    ...
                    row.view.setOnClickListener(v -> {
                            ...
                            provideTouchHapticH(VibrationEffect.get(VibrationEffect.EFFECT_POP));
                    });
                ...
                if (mMediaOutputView.indexOfChild(row.view) == -1) {
    ...
    private MediaOutputRow findMediaOutputRow(MediaDevice device) {
        for (MediaOutputRow row : mMediaOutputRows) {
            if (row.device.getId().equals(device.getId())) {
                return row;
            }
        }
        return null;
    ...
    private void setSelectedMediaRows(boolean sel) {
        for (MediaOutputRow row : mMediaOutputRows) {
            row.name.setSelected(sel);
            row.summary.setSelected(sel);
        }
    }

    public void initRingerH() {
        if (mRingerIcon != null && !mHasAlertSlider) {
    ...
    protected void rescheduleTimeoutH() {
        mHandler.removeMessages(H.DISMISS);
        final int timeout = computeTimeoutH();Log.d(TAG, "rescheduleTimeout " + timeout);
    ...
    private void enableRingerViewsH(boolean enable) {
        if (mRingerIcon != null) {
            mRingerIcon.setEnabled(mHasAlertSlider ? false : enable);
        }
        if (mZenIcon != null) {
    ...
    private void updateVolumeRowSliderH(VolumeRow row, boolean enable, int vlevel) {
        ...
        // update header text
        Util.setText(row.header, Utils.formatPercentage(enable ? vlevel : 0, row.ss.levelMax));
    }
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if (mRow.ss.level != userLevel || mRow.ss.muted && userLevel > 0) {
                ...
                if (mRow.requestedLevel != userLevel) {
                    ...
                    if (mVibrateOnSlider && !mHandlerSecondary.hasMessages(H.PERFORM_HAPTIC_FEEDBACK)) {
                        mHandlerSecondary.sendEmptyMessageDelayed(H.PERFORM_HAPTIC_FEEDBACK, 2);
                    }
                }
            }
        }
