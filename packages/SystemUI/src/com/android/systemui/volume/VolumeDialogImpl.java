...
public class VolumeDialogImpl implements VolumeDialog, Dumpable,
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        final boolean zenMuted = isZenAlarms ? (isRingStream || isNotificationStream || isSystemStream)
                : isZenNone ? (isRingStream || isNotificationStream || isSystemStream || isAlarmStream || isMusicStream)
        ...
        } else if (isRingSilent || isNotificationSilent || isNotificationStream && isStreamMuted(ss)
                || zenMuted) {
        ...
        if (zenMuted || isNotificationVibrate || isNotificationSilent) {
            row.tracking = false;
        }
        enableVolumeRowViewsH(row, !zenMuted);

        // update slider
        final boolean enableSlider = !zenMuted && !isNotificationVibrate && !isNotificationSilent;
        final int vlevel = row.ss.muted && (!isRingStream && !zenMuted && !isNotificationVibrate
                && !isNotificationSilent && !(isNotificationStream && isStreamMuted(ss))) ? 0
                : row.ss.level;
