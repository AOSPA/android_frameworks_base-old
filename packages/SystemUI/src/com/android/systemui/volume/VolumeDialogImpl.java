...
public class VolumeDialogImpl implements VolumeDialog,
            ...
            mExpandRows.setOnClickListener(v -> {
                if (!mExpanded) {
                    ...
                    if (mTelephonyManager.getCallState() == TelephonyManager.CALL_STATE_IDLE) {
                        ...
                        // Translate from -2*width to 0 and move Z coordinate from 0 to mElevation
                        // where all coordinates are with respect to the view's coordinates
                    ...
                    if (alarm != null) {
                        ...
                        // If alarm is currently active the other bars don't need to animate from that far out
                        width = (alarm == active) ? width/2 : width;
                    }
                    if (ring != null) {
                        Util.setVisOrGone(ring.view, true);
                        // Translate from -width to 0 and move Z coordinate from mElevation/2 to mElevation
                        animateView(ring.view, true, width, mElevation/2, ring == active);
                        width = (ring == active) ? width : width * 2;
                    }
    ...
    private void animateView(View view, boolean slideIn, float negX, float startZ, boolean viewStays) {
        ...
        if (slideIn) {
            ...
            view.animate()
                .alpha(1)
                // animate to the default X coordinate of this view
                .translationX(0)
                // animate to the default Z coordinate(mElevation in this case)
                // this is set in the xml layout for all views
                .translationZ(0)
                .setDuration(DIALOG_SHOW_ANIMATION_DURATION)
                .setInterpolator(new SystemUIInterpolators.LogDecelerateInterpolator())
                .withEndAction(() -> {
            ...
            view.animate()
                .alpha(viewStays ? 1 : 0)
                .translationX(-negX)
                // animate vertically down by "startZ"
