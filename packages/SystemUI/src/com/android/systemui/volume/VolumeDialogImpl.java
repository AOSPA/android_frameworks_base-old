...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private final HandlerThread mThreadVibrate = new HandlerThread("HapticsProvider");
    private final H mHandlerVibrate;
    ...
    public VolumeDialogImpl(Context context) {
        ...
        if (mVibrateOnSlider) {
            mThreadVibrate.start();
            mHandlerVibrate = new H(mThreadVibrate.getLooper());
        } else {
            mHandlerVibrate = null;
        }
        setDarkMode();
    }
    ...
    private void initDialog() {
        ...
        mMediaOutput = mDialog.findViewById(R.id.media_output);
        Util.setVisOrGone(mMediaOutput, false);
    ...
    private void cleanExpandedRows() {
        ...
        if (alarm != null) {
            animateRow(alarm, false, 0, active == alarm);
            width = (alarm == active) ? width/2 : width;
        }
        if (ring != null) {
            animateRow(ring, false, width, active == ring);
            width = (ring == active) ? width : width * 2;
        }
        animateRow(media, false, width, true);
        Util.setVisOrGone(mMediaOutput, false);
    }
            ...
            mExpandRows.setOnClickListener(v -> {
                if (!mExpanded) {
                    ...
                    if (alarm != null) {
                        ...
                        animateRow(alarm, true, 0, alarm == active);
                        width = (alarm == active) ? width/2 : width;
                    }
                    if (ring != null) {
                        Util.setVisOrGone(ring.view, true);
                        animateRow(ring, true, width, ring == active);
                        width = (ring == active) ? width : width * 2;
                    }
                    animateRow(media, true, width, true);
                    Util.setVisOrGone(mMediaOutput, true);
                    mMediaOutput.setSelected(true);
                    mExpanded = true;
                } else {
                    cleanExpandedRows();
                    mExpanded = false;
    ...
    private void animateRow(VolumeRow row, boolean slideIn, float negX, boolean rowStays) {
        if (slideIn) {
            row.view.setTranslationX(-negX);
            row.view.setAlpha(rowStays ? 1 : 0);
            row.view.animate()
                .alpha(1)
                .translationX(0)
                .setDuration(DIALOG_SHOW_ANIMATION_DURATION*2)
                .setInterpolator(new SystemUIInterpolators.LogDecelerateInterpolator())
                .withEndAction(() -> {
                    row.view.setTranslationX(0);
                })
                .start();
        } else {
            row.view.setAlpha(1);
            row.view.animate()
                ...
                .setInterpolator(new SystemUIInterpolators.LogDecelerateInterpolator())
                .withEndAction(() -> {
                    Util.setVisOrGone(row.view, rowStays);
                    row.view.setTranslationX(0);
                    row.view.setAlpha(1);
                })
                .start();
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if (mRow.ss.level != userLevel || mRow.ss.muted && userLevel > 0) {
                ...
                if (mRow.requestedLevel != userLevel) {
                    ...
                    if (mVibrateOnSlider && !mHandlerVibrate.hasMessages(H.PERFORM_HAPTIC_FEEDBACK)) {
                        mHandlerVibrate.sendEmptyMessageDelayed(H.PERFORM_HAPTIC_FEEDBACK, 2);
                    }
                }
            }
        }
