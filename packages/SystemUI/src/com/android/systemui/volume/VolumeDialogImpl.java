...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private void cleanExpandedRows() {
        ...
        if (alarm != null) {
            final boolean isAlarmVisible = shouldBeVisibleH(alarm, active);
            animateViewOut(alarm.view, isAlarmVisible, 0, z);
            z /= 2;
            width = isAlarmVisible ? width / 2 : width;
        }
        if (ring != null) {
            final boolean isRingVisible = shouldBeVisibleH(ring, active);
            animateViewOut(ring.view, isRingVisible, width, z);
            z /= 2;
            width = isRingVisible ? width : width * 2;
        }
        if (media != null) {
            final boolean isMediaVisible = shouldBeVisibleH(media, active);
            animateViewOut(media.view, isMediaVisible, width, z);
        }
        ...
        if (mExpandRows != null) {
            mMediaButton.setOnClickListener(v -> {
                if (mShowingMediaDevices) {
                    ...
                    mCurrAnimator = circularReveal(mMediaOutputScrollView);
                    mCurrAnimator.start();
                }
            ...
            mExpandRows.setOnClickListener(v -> {
                if (!mExpanded) {
                    ...
                    if (showMediaOutput) {
                        animateViewIn(mMediaButtonView, false, width * 2, z);
                    }
                    ...
                    if (media != null) {
                        final boolean isMediaVisible = shouldBeVisibleH(media, active);
                        animateViewIn(media.view, isMediaVisible, width, z);
                    }
    ...
    private Animator circularReveal(View view) {
        ...
        VolumeRow active = getActiveRow();
        float height = (float) active.view.getHeight();
        float width = (float) active.view.getWidth();
        float spacer = (float) mContext
            .getResources().getDimension(R.dimen.volume_dialog_row_spacer);

        int x = (int) (isLandscape() ? (isAudioPanelOnLeftSide() ? (width + spacer) * 2 +
            (mHasAlertSlider ? width/2 : width ) : (mHasAlertSlider ? width/2 : 0 )) : (1.5 * width + spacer));
        int endRadius = (int) Math.hypot((isLandscape() ? 2.2 : 1.1 ) * (1.5 * width + spacer), height);

        Animator anim = ViewAnimationUtils.createCircularReveal(view, x,
            isLandscape() ? 0 : (int) height, 0, endRadius);
    ...
    private Animator circularExit(View view) {
        if (view == null) return null;
        ...
        Animator anim = ViewAnimationUtils.createCircularReveal(view, x,
            isLandscape() ? 0 : (int) height, endRadius, 0);
            ...
            if (mMediaDevices.contains(row.device)) {
                if (row.device.isConnected()) {
                    ...
                    if (row.device.getSummary() != null) {
                        ...
                        Util.setVisOrGone(row.selected, row.device.getSummary().contains("Active"));
                    } else {
        ...
        if (mState != null) {
            ...
            switch (mState.ringerModeInternal) {
                ...
                default:
                    ...
                    if (mHasAlertSlider) {
                        row.slider.setProgress(ss.level * 100, true);
                        Util.setText(row.header, Utils.formatPercentage(ss.level, ss.levelMax));
                    }
                    if (!isZenMuted && muted) {
                        mRingerIcon.setImageResource(R.drawable.ic_volume_ringer_mute);
    ...
    private final class VolumeSeekBarChangeListener implements OnSeekBarChangeListener {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            if (mRow.stream == STREAM_RING && mRow.ss.muted && mHasAlertSlider) {
