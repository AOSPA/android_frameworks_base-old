...
public class AppLockUi {
    ...
    private SecureAppsService mService;
    ...
    public AppLockUi(SecureAppsService service, Context context, String packageName, int userId) {
        mContext = context;
    ...
    private void loadUi() {
        ...
        if (icon != null) {
            ...
            textColor = ColorUtils.isColorDark(backgroundColor) ?
                    mContext.getColor(com.android.internal.R.color.white) : mContext.getColor(com.android.internal.R.color.black);
    ...
    public boolean updateConfig(Configuration newConfig) {
        ...
        if (mRotation != rot) {
            ...
            if (mLayoutChanged) {
                ...
                if (N > 0) {
                    show(mWindowTokens.get(N-1), mIsAppWindowVisible);
                    return true;
                }
            }
        }
    ...
    public void onAppLaunching() {
        ...
        final int N = mWindowTokens.size();
        if (N > 0) show(mWindowTokens.get(N-1), mIsAppWindowVisible);
    }
    ...
    public void notifyTimerTick() {
        String text = mContext.getString(
                R.string.remaining_attempts_pattern, (int) (sCountdown / 1000));
    ...
    public static class SecureAppsThread extends HandlerThread {
        ...
        private static void ensureThreadLocked() {
            if (sInstance == null) {
                sInstance = new SecureAppsThread("android.display_secureapps", -4 /*THREAD_PRIORITY_DISPLAY*/ + 1);
                sInstance.start();
