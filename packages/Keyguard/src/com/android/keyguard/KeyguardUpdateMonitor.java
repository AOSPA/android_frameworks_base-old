...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private void handleFingerprintAcquired() {
        if (!mDeviceInteractive && mProximityRange == PROXIMITY_NEAR) {
            ...
            mHandler.postDelayed(mUnregisterProximitySensor, 500);
            return;
        }
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
    ...
    private void updateFingerprintListeningState() {
        ...
        boolean proximityRangeNotReadyOrNear = mProximityRange == PROXIMITY_NEAR || mProximityRange == PROXIMITY_NOT_INITIALIZED;
        if (!mDeviceInteractive && proximityRangeNotReadyOrNear && mFingerprintRunningState != FINGERPRINT_STATE_STOPPED) {
            setFingerprintRunningState(FINGERPRINT_STATE_STOPPED);
